...
//	$selectSql = "select  m.term_id,m.name,sum(CONVERT(pm.meta_value,UNSIGNED))sumhit,m.slug,u.user_login from wp_terms m,wp_term_taxonomy t,wp_term_relationships r,wp_posts p,wp_postmeta pm ,wp_users u where m.term_id = t.term_id and t.taxonomy='series' and r.term_taxonomy_id=t.term_taxonomy_id and r.object_id=p.id and p.id=pm.post_id and pm.meta_key='custom_total_hits' and p.post_author=u.id group by  m.term_id, m.name,m.slug,u.user_login order by sumhit desc limit $showNum";
	...
	$selectSql = <<<SQL
	select  m.term_id,m.name,sum(CONVERT(pm.meta_value,UNSIGNED))sumhit,m.slug,u.user_login from wp_terms m,
	wp_term_taxonomy t,wp_term_relationships r,wp_posts p,wp_postmeta pm ,
	wp_users u,wp_orgseriesicons o where m.term_id = t.term_id and t.taxonomy='series' and r.term_taxonomy_id=t.term_taxonomy_id 
	and r.object_id=p.id and p.id=pm.post_id and pm.meta_key='custom_total_hits' and m.term_id=o.term_id and o.user_id=u.id 
	group by  m.term_id, m.name,m.slug,u.user_login order by sumhit desc limit $showNum
SQL;
...
function getHighestRationDatas ($showNum){
   global $wpdb;  
	...
	where m.term_id=t.term_id and t.taxonomy='series' and t.term_taxonomy_id=r.term_taxonomy_id
	 and r.object_id=p.id and p.id=rating_postid and m.term_id=o.term_id and 
	 o.user_id=u.id group by m.name,m.slug,m.term_id,u.user_login order by avgrate desc limit $showNum
SQL;
